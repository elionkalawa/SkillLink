...
import { X, Loader2, Plus, Trash2, MapPin, ChevronDown, Tag, Users, Pencil } from "lucide-react";
...
export default function EditProjectModal({ project, onClose }: EditProjectModalProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    category: project.category || ("Productivity" as ProjectCategory),
    location: (project.location || "Remote") as Location,
  ...
  const [roles, setRoles] = useState<UIRole[]>(
    project.roles
      ? project.roles.map((r) => ({
          ...r,
          skills_required: Array.isArray(r.skills_required)
            ? r.skills_required.join(", ")
            : ((r.skills_required as unknown as string) || ""),
        }))
      : [],
  );
